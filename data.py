...
def f_append(lst, item):
    """
    do list += item as a function
    """
    return lst.__iadd__(item)
...
def replace_at_positions(
    ...
    """
    after_ids: list = random.sample(
        range(len(input)), k=num_replacements)
    return [
        insert_text if i in after_ids 
        else x 
        for i, x in enumerate(input)]

def mask_text_with_func(
        text,
...
def group_list_of_dicts_by_key(
    ...
    raise the values of a key. 
        {a:1, b:0}, ..., {a:0, b:1}
        (values of a into keys)
           -> {1: {...}, 2: {...}}
    """
    grouped_data = {}
    sorted_iterable = sorted(iterable, key=key_func)

    for key, group in toolz.groupby(
            key=key_func,
            seq=sorted_iterable).items():
...
def downsample_dataset(
    ...
    label_distribution = toolz.valmap(
        len, partitioned_dataset)
    _, num_samples = sorted(
        label_distribution.items(), key=lambda x: x[1])[0]

    data = toolz.valmap(
        functools.partial(
            sample_func, k=num_samples), 
        partitioned_dataset)
    return itertools.chain.from_iterable(data.values())


def format_dict_batch(
        batch: list[Any], 
        key_order: list[str],
    ...
    for key, process in key_funcs.items():
        ...
        if len(apply_to_keys) == 1: # applying function to values from ONE key
            ...
            output[key] = [
                func(*vals) for vals in 
                zip(*toolz.keyfilter(
                    lambda x: x in apply_to_keys, ks_vs).values())
            ]
    return output

def get_sampled_batches_from_dataset(
        data: list,
        batch_size: int, 
        sample_func: Callable,
    ...
    for _ in range(num_steps):
        batch = sample_func(
            data, **sample_func_kwargs)
        if batch:
            yield batch_filter_func(
                batch, **batch_filter_func_kwargs)
    
def get_linear_batches_from_dataset(
        data: list, 
        batch_size: int, 
        batch_filter_func: Callable = lambda x: x,
        do_shuffle: bool = True,
